...
import StudentHasilTO from '../student/HasilTO';
import { muted, fmtDateTime, ToTabs } from './shared';
import type { ToPackage } from '../../types';

// Same grades gate the old standalone "Hasil TO" sidebar item used
// to apply -- only these grades actually sit real SNBT/TKA tryouts,
// so the Sertifikat tab stays hidden (not just empty) for the rest.
...
export default function ToPaketListTaker() {
  ...
  const showSertifikat = profile?.role === 'student' && TO_GRADES.includes(profile?.tingkat_kelas ?? '');
