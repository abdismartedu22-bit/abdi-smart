...
export default function StudentQuiz() {
  ...
  if (phase === 'already-done' && prevScore) {
                    ...
                    return (
                      <label key={oi} style={{ ...optionRow, background: checked ? '#EDE9FE' : '#F9F9F7', borderColor: checked ? '#7C3AED' : '#E2E1DC', cursor: 'pointer' }}>
                        <input
                          type="checkbox"
                          checked={checked}
                          onChange={() => toggleCheckbox(q.id, label)}
                          style={{ flexShrink: 0 }}
                        />
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
      <span style={{
        ...
        background: '#0D5C3A', padding: '3px 10px', borderRadius: '6px', flexShrink: 0,
      }}>
...
const pageTitle: React.CSSProperties = {
  fontFamily: 'var(--font-display)', fontSize: '1.6rem', margin: '0 0 20px', color: '#0D0D0D',
};
...
const inputStyle: React.CSSProperties = {
  ...
  color: '#0D0D0D', background: '#fff', boxSizing: 'border-box',
};
